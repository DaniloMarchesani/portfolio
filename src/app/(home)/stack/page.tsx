import { ChevronLeft, ChevronsRight, Logs } from "lucide-react";
...
export default function MyStack() {
    ...
    return (
        ...
        <Link href="/" className="font-light text-sm text-neutral-500 hover:text-blue-500"><div className="flex items-center"><ChevronLeft className="h-4 w-4" />navigate back</div></Link>
        <h2 className="text-lg font-medium mb-8 tracking-tighter mt-8">
            My Stack
        </h2>
        <h3 className="text-neutral-300 mb-10">Here's where I'm currently most productive:</h3>
        { stacks.map( (stack, index) => (<div key={index} className="mb-16">
                ...
                <div className="flex flex-col gap-4 mt-4">
                    {stack.descriptions.map((description, index) => (
                        <div key={index} className="flex items-start justify-start gap-2">
                            <div><ChevronsRight className="h-4 mt-1 opacity-50"/></div>
